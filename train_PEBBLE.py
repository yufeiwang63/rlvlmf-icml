...
class Workspace(object):
    def __init__(self, cfg):
        ...
        # image_height = image_width = 480
        image_height = image_width = cfg.image_size
        if "sweep" in cfg.env or 'drawer' in cfg.env:
            image_height = image_width = 300 # to zoom in further into the scene
    ...
    def evaluate(self, save_additional=False):
        ...
        if self.cfg.env == 'dressing':
            for key, val in utils.get_info_stats(all_ep_infos).items():
        ...
        while self.step < self.cfg.num_train_steps:
            ...
            if done:
                if self.step > 0:
                    ...
                    if self.cfg.env == 'dressing':
                        for key, val in utils.get_info_stats([ep_info]).items():
            ...
            if self.cfg.image_reward and self.reward not in ["gt_task_reward", "sparse_task_reward"]:
                self.replay_buffer.add(obs, action, reward_hat, 
                    next_obs, done, done_no_max, image=rgb_image)
            else:
                self.replay_buffer.add(obs, action, reward_hat, 
